 ...
 * @LastEditTime: 2022-03-17 16:31:35
 * @FilePath: /snow-wolf/1、bagu/RegExp/demo5.js
 * @Description: file information
 */
// 改变输入字符串的顺序
// 以下例子解释了正则表达式的构成和 string.split() 以及 string.replace()的用途。
...
for (i = 0, len = nameList.length; i < len; i++) {
  ...
  bySurnameList[i] = nameList[i].replace(pattern, "$2, $1"); // 按空格分隔并调换前后的位置（以逗号为分界线）
}
...
// 根据姓来排序，然后输出排序后的数组。
bySurnameList.sort();
console.log('[ bySurnameList 根据姓来排序，然后输出排序后的数组。] >', bySurnameList)
output.push("---------- Sorted");
for (i = 0, len = bySurnameList.length; i < len; i++) {
  output.push(bySurnameList[i]);
}
